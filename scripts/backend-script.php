...
if( !empty($_GET['id']) && !empty($_GET['tableName']) && $_GET['role']=='bookie'){
  $id=legal_input($_GET['id']);
  $tableName=legal_input($_GET['tableName']);
  $query="SELECT status FROM bookie_profile WHERE id=$id";
  $res= $db->query($query);
  $role=$res->fetch_assoc();
  if($role['status']==1)
  {
...
if(empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='bookie_profile'){
  extract($_POST);
 if(!empty($full_name) & !empty($password)){
  if($password==$cpassword)
  {
   ...
   if(!empty($data) && !empty($tableName)){
      $insertData=insert_data($data,$tableName);
      if($insertData){
        echo "<span class='success'>Bookie Profile saved sucessfully</span>";
      }else{
        echo "<span class='fail'>Error!.. check your query</span>";
      }
   }
 ...
 echo "<span class='fail'>Bookie Profile field is empty</span>";
}
 ...
 if(!empty($full_name) & !empty($password)){
  if($password==$cpassword)
  {
   ...
   if(!empty($data) && !empty($tableName)){
      ...
      if($updateData){
        echo "<span class='success'>Booie Profile saved sucessfully</span>";
      }else{
        echo "<span class='fail'>Error!.. check your query</span>";
      }
   }
...
if(!empty($_GET['name']) && $_GET['name']=='new_bookie'){
  extract($_POST);
 if(!empty($full_name) && !empty($email) & !empty($password)){
    $data= [
      'full_name' =>legal_input($full_name),
   ...
   if(!empty($data) && !empty($tableName)){
      $insertData=insert_data($data,$tableName);
      if($insertData){
        echo "<span class='success'>New Bookie saved sucessfully</span>";
      }else{
        echo "<span class='fail'>Error!.. check your query</span>";
      }
   }
...
if(!empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='update_new_bookie')
{
 ...
 if(!empty($full_name) && !empty($email) & !empty($password)){
     $data= [
       'full_name' =>legal_input($full_name),
       'email' =>legal_input($email),
       'balance' =>legal_input($balance),
       'password'=>md5($password)
     ];
  
  $updateData=update_data($data,$tableName, $id);
   if($updateData){
    echo "<span class='success'>Bookie updated sucessfully</span>";
   }else{
    echo "<span class='fail'>Error!.. check your query</span>";
   }
