...
$(document).on('click', '.bookieRole', function(e){
  ...
  var id=$(this).attr('rel');
   ...
   method:"GET",
   url: "scripts/backend-script.php",
   data:{tableName:name,id:id,role:'bookie'},
   ...
   success: function(data){
     ...
     {
       case 'fas fa-user-alt-slash iconRole':
       el.attr('class', 'text-secondary bookieRole');
       break;
       case 'fas fa-user-alt iconRole':
       el.attr('class', 'text-success bookieRole');
       break;
     }
...
$(document).on('submit','#bookieForm',function(e){
  e.preventDefault();
 var name=$(this).attr('name');
  $.ajax({
  method:"POST",
  ...
  $('#bookieForm').find('input').val('')
  $('#bookieForm').find('textarea').val('');
  $('button[type="submit"]').removeAttr('disabled').text('Save');
...
$(document).on('submit','#updateForm',function(e){
      ...
      $(document).on('submit','#updateBookieForm',function(e){
        e.preventDefault();
       var formData = new FormData(this);
       var name=$(this).attr('name');
       var id= $(this).attr('rel');
